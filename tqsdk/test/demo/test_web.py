...
class WebTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.ins = MockInsServer(5000)
        cls.chrome_options = ChromeOptions()
        cls.chrome_options.headless = True
        cls.firefox_options = FirefoxOptions()
        cls.firefox_options.headless = True
        ctx = mp.get_context('spawn')
        cls.port = "8084"
        cls.tq_process = ctx.Process(target=run_tianqin_code, args=(cls.port,))
        cls.tq_process.start()
        time.sleep(10)

    @classmethod
    def tearDownClass(cls):
        cls.ins.close()
        cls.tq_process.terminate()

    @unittest.skipIf(not sys.platform.startswith("win"), "test on win")
    def test_chrome_on_win(self):
